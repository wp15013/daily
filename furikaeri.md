...
- 今日学んだ事・・・・
- Stringオブジェクト（文字列もオブジェクト）・lengthプロパティ・charAtメソッド
- Stringオブジェクト(リンクや文字サイズ設定)・linkメソッド・fontsizeメソッド
- 配列データの管理
- 配列と添字
- 配列はArrayオブジェクト
- 曜日を日本語で表示する
- Arrayオブジェクトのメソッド・joinメソッド・sortメソッド
- 


### problem(問題なので、やめること)

- 1日で新たに学んだ事を忘れない為にはどうした方が良いのか？
- それとも、今の時期は、こんな感じで、徐々に身について行くものなのかな？

### Try(今後、試してみたいこと)

- 確かに、配列は重要な感じがしたので、しっかりと理解したい！
...
- 今日学んだ事・・・・
- ・function(ユーザー関数定義)
- ・変数のスコープ・ローカル変数・グローバル変数の優先順位
- ・オブジェクトの生成・インスタントの生成・コンストラクタ
- ・Dateオブジェクトの生成
- 日付・時刻の操作
- ・Dateオブジェクトのメソッド
- ・Mathオブジェクト
...
- ローカル変数とグローバル変数のスコープ？優先順位？がイマイチ理解出来ていない！
- コンストラクタとは・・・・覚えて・・・・？
